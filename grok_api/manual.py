...
def main():
    ...
    data2 = Grok(proxy=proxy).chat(message2, extra_data=data1["extra_data"])
    Log.Info("GROK: " + data2["response"])

    message3: str = "crazy"
    Log.Info("USER: " + message3)
    data3 = Grok(proxy=proxy).chat(message3, extra_data=data2["extra_data"])
    Log.Info("GROK: " + data3["response"])

    message4: str = "Well this is the 4th message in our chat now omg"
    Log.Info("USER: " + message4)
    data4 = Grok(proxy=proxy).chat(message4, extra_data=data3["extra_data"])
    Log.Info("GROK: " + data4["response"])

    message5: str = "And now the 5th omg"
    Log.Info("USER: " + message5)
    data5 = Grok(proxy=proxy).chat(message5, extra_data=data4["extra_data"])
